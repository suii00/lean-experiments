...
\lstdefinelanguage{lean4}{
  morekeywords={def,theorem,lemma,example,instance,class,structure,
    ...
    refine,change,subst,cases,fin_cases,
    attribute,deriving},
  sensitive=true,
  morecomment=[l]{--},
  morecomment=[n]{/-}{-/},
...
$\{T_i\}_{i \in \iota}$（\emph{構造塔}）と、それを拡張した
\emph{網羅的塔}（ExhaustiveTower）および \emph{特性ランク}（CharRank）である。
...
\# & カテゴリ名 & 主要な内容 \\
\midrule
1 & 基本的構成 & \texttt{iic}, \texttt{natFiltration}, \texttt{ofAntitone}, \texttt{inter} \\
2 & 空の塔 & \texttt{emptyTower}, 空集合の性質 \\
3 & 網羅的塔とランク & \texttt{natExhaustive}, \texttt{HasCharRank}, ランク一意性 \\
4 & 順序数による塔 & $\omega$-bounded ordinal tower \\
5 & 反例：単調性の失敗 & \texttt{FakeTower}, \texttt{fakeExample} \\
6 & 前順序でのランク非一意性 & \texttt{TwoPoint}, \texttt{rank\_not\_unique} \\
7 & 双対順序とイデアル冪 & \texttt{idealPowTower}, $I^n \supseteq I^{n+1}$ \\
8 & シングルトンと層分解 & \texttt{singletonTowerFails}, \texttt{natFiltration\_layer} \\
9 & 整数のフィルトレーション & \texttt{almostFiltered}, 加法の非閉性 \\
10 & 最終的単調列 & \texttt{EventuallyMonotoneSeq}, \texttt{truncateToTower} \\
11 & 有限・閾値塔 & \texttt{thresholdTower}, \texttt{finTower1}, \texttt{finTower2} \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{構造塔} $T$ とは、以下のデータからなる構造体である：
\begin{itemize}
  \item 層関数 $\lev_T : \iota \to \mathcal{P}(\alpha)$（$\mathcal{P}$ はべき集合）
  \item 単調性：$\forall i, j \in \iota,\; i \leq j \implies \lev_T(i) \subseteq \lev_T(j)$
\end{itemize}
型 $\mathsf{StructureTower}\,\iota\,\alpha$ で表す。
\end{definition}

\begin{definition}[union, global]\label{def:union-global}
構造塔 $T : \mathsf{ST}\,\iota\,\alpha$ に対し：
\[
...
$T : \mathsf{ST}\,\mathbb{N}\,\alpha$ が \emph{網羅的} であるとは，
$\forall x \in \alpha,\, \exists n \in \mathbb{N},\, x \in \lev_T(n)$，
すなわち $\union(T) = \alpha$ が成り立つことをいう。
このとき $T$ を $\mathsf{ExhaustiveTower}\,\alpha$ と呼ぶ。
\end{definition}

\begin{definition}[HasCharRank]\label{def:charrank}
$T : \mathsf{ET}\,\alpha$，$r : \alpha \to \mathbb{N}$ とする。
$r$ が $T$ の \emph{特性ランク関数} であるとは：
\[
...
% =============================================================================
\section{カテゴリ 1：基本的な構成}\label{sec:basic}
% =============================================================================

\subsection{下切り塔 \texttt{iic}}

\begin{definition}[下切り塔]\label{def:iic}
前順序集合 $\alpha$ に対し，\emph{下切り塔} $\mathsf{iic}(\alpha)$ を
\[
  \lev_{\mathsf{iic}}(x) := \{y \in \alpha \mid y \leq x\} = (-\infty, x]
\]
で定義する。これは $\mathsf{ST}\,\alpha\,\alpha$ の元である。
\end{definition}

\begin{proposition}
$\mathsf{iic}(\alpha)$ は構造塔の単調性条件を満たす。
\end{proposition}
\begin{proof}
$x \leq z \in \lev_{\mathsf{iic}}(x)$ かつ $x \leq y$ とする。
推移律より $z \leq y$，したがって $z \in \lev_{\mathsf{iic}}(y)$。
\end{proof}

\begin{leancode}[caption={iic の定義}]
...
特殊ケースとして，$\mathsf{natFiltration} := \mathsf{iic}(\mathbb{N})$ を
\emph{自然数フィルトレーション}と呼ぶ。
これは $\lev_n = \{0, 1, \ldots, n\}$ を与える最も標準的な塔である。

\subsection{反変族から塔を構成する \texttt{ofAntitone}}

自然な順序に関して単調減少する集合族（反変族）からも構造塔が作れる。
鍵となるのは \emph{双対順序}（order dual）$\iota^{\mathrm{op}}$ の利用である。

\begin{definition}[ofAntitone]\label{def:ofantitone}
$d : \iota \to \mathcal{P}(\alpha)$ が反変（$i \leq j \implies d(j) \subseteq d(i)$）
であるとき，
\[
  \mathsf{ofAntitone}(d) : \mathsf{ST}\,\iota^{\mathrm{op}}\,\alpha, \qquad
  \lev(i) := d(\bar{i})
\]
と定義する。ここで $\bar{i}$ は双対順序における $i$ の像である。
\end{definition}
...
\begin{tikzcd}[column sep=large]
  \iota^{\mathrm{op}}
    ...
    \arrow[d, "\mathrm{ofDual}"']
  & \mathcal{P}(\alpha) \\
  \iota
    \arrow[ur, "d"']
\end{tikzcd}
\caption{\texttt{ofAntitone} の可換図式。
  双対順序 $\iota^{\mathrm{op}}$ 上では単調、
  元の順序 $\iota$ 上では反変。}
\label{fig:ofantitone}
\end{figure}

\begin{leancode}[caption={ofAntitone の定義}]
...
$T_1, T_2 : \mathsf{ST}\,\iota\,\alpha$ に対し，
\[
  (T_1 \cap T_2)_i := \lev_{T_1}(i) \cap \lev_{T_2}(i)
\]
と定義された塔を \emph{交差塔} \texttt{inter}$(T_1, T_2)$ と呼ぶ。
\end{definition}
...
% =============================================================================
\section{カテゴリ 2：空の塔}\label{sec:empty}
% =============================================================================
...
\]
と定義する。空集合は任意の集合に包含されるので単調性は自明に成立する。
...
単調性の証明は \texttt{cases hx}（\texttt{hx : x ∈ ∅}）で即座に終わる。
これは空虚真（vacuous truth）の実装例である。
\end{mathinsight}

\begin{leancode}[caption={空の塔と基本定理}]
...
% =============================================================================
\section{カテゴリ 3：網羅的塔とランク}\label{sec:exhaustive}
% =============================================================================

\subsection{自然数フィルトレーションの網羅性}

\begin{proposition}
$\mathsf{natExhaustive} := \mathsf{iic}(\mathbb{N})$ は網羅的塔であり，
特性ランク関数は恒等写像 $r = \mathrm{id}_\mathbb{N}$ である。
\end{proposition}
\begin{proof}
任意の $x \in \mathbb{N}$ に対し $x \in \lev_x = \{y \mid y \leq x\}$（$x \leq x$）
なので網羅性が成立。ランクについては
$x \in \lev_n \iff x \leq n \iff \mathrm{id}(x) \leq n$。
\end{proof}

\begin{theorem}[ランク一意性（部分順序の場合）]\label{thm:rank-unique}
$T : \mathsf{ET}\,\alpha$ の添字型 $\mathbb{N}$ が\textbf{部分順序}（反対称律付き）
であるとき，特性ランク関数は一意である：
\[
...
\begin{mathinsight}
証明の核心は，$r_1(x) \leq r_2(x)$ かつ $r_2(x) \leq r_1(x)$ を示し，
自然数の反対称律（$\mathbb{N}$ は部分順序）から $r_1(x) = r_2(x)$ を導くことである。
\end{mathinsight}

\begin{leancode}[caption={natExhaustive の定義とランク等式}]
...
% =============================================================================
\section{カテゴリ 4：順序数による塔}\label{sec:ordinal}
% =============================================================================

\subsection{$\omega$-有界順序数による自然数の塔}

\begin{definition}[ordinalNatBound]
$o < \omega_0$（最初の極限順序数）のとき，$o$ に対応する自然数
$\mathsf{ordinalNatBound}(o) \in \mathbb{N}$ を $o = \overline{n}$（$n \in \mathbb{N}$）
となる $n$ の古典的選択で定める。
\end{definition}

\begin{definition}[ordinalTower]
$\mathsf{ordinalTower} : \mathsf{ST}\,\mathsf{Ord}\,\mathbb{N}$ を
\[
  \lev(o) :=
  \begin{cases}
...
\begin{tikzpicture}[scale=0.9,
  ...
  \node[above] at (4.8, 2.0) {\scriptsize $\omega_0$};
\end{tikzpicture}
\caption{\texttt{ordinalTower} の層の様子。
  $o < \omega_0$ では有限層、$o \geq \omega_0$ では全体 $\mathbb{N}$。
  和集合は $\mathbb{N}$ 全体に等しい。}
...
\end{theorem}
\begin{proof}
任意の $x \in \mathbb{N}$ に対し，$x < \omega_0$（Mathlib の補題 \texttt{nat\_lt\_omega0}）
と $\mathsf{ordinalNatBound}(x) = x$ より $x \in \lev(x)$。
\end{proof}

% =============================================================================
\section{カテゴリ 5：反例 — 単調性の失敗}\label{sec:fake}
% =============================================================================
...
\texttt{FakeTower} は \texttt{monotone\_level} フィールドを持たない ``偽の塔'' 構造体であり，
単調性を満たさない集合族の存在を示す反例として定義される。
Lean での証明は：$0 \in S_0$ かつ $S_0 \subseteq S_1$ を仮定すると
$0 \in S_1 = \{2\}$ となり矛盾（\texttt{simp} で解決）。
\end{counterpointbox}

% =============================================================================
\section{カテゴリ 6：前順序でのランク非一意性}\label{sec:preorder}
% =============================================================================

\subsection{二点前順序}

\begin{definition}[TwoPoint 前順序]\label{def:twopoint}
集合 $\{a, b\}$ 上に全ての対で $x \leq y$ が成立する前順序（全関係）を与える。
すなわち $a \leq b$ かつ $b \leq a$ であるが $a \neq b$（反対称律が成立しない）。
\end{definition}

\begin{theorem}[前順序ではランクが非一意]
\texttt{twoPointExhaustive}（全層が $\mathbb{N}$ である網羅的塔）に対し，
特性ランク関数 $r_1 \equiv a$ および $r_2 \equiv b$ はいずれも
\texttt{HasCharRankPreorder} を満たすが $r_1 \neq r_2$ である。
\end{theorem}

\begin{figure}[H]
\centering
\begin{tikzcd}[column sep=3cm]
  \mathbb{N} \arrow[r, bend left=15, "r_1 \equiv a"]
             \arrow[r, bend right=15, "r_2 \equiv b"']
  & \{a, b\}
\end{tikzcd}
\caption{二点前順序でのランク非一意性。$r_1, r_2$ はともに特性ランクの条件を満たすが異なる写像。}
\label{fig:rank-nonunique}
\end{figure}

\begin{mathinsight}
ランク一意性（定理~\ref{thm:rank-unique}）が成立するのは添字型が
\textbf{部分順序}（反対称律あり）のときに限る。
前順序では $r_1(x) \leq r_2(x)$ かつ $r_2(x) \leq r_1(x)$ から
$r_1(x) = r_2(x)$ を導けない。
今回の例では $a \leq b \leq a$ だが $a \neq b$ というまさにそのケースである。
\end{mathinsight}

% =============================================================================
\section{カテゴリ 7：双対順序とイデアル冪}\label{sec:ideal}
% =============================================================================
...
$\lev_n := I^n$（$I$ の $n$ 乗イデアル）とする集合族は \emph{反変}：
\[
  m \leq n \implies I^n \subseteq I^m.
\]
双対順序 $\mathbb{N}^{\mathrm{op}}$ を添字として
$\mathsf{idealPowTower}(I) : \mathsf{ST}\,\mathbb{N}^{\mathrm{op}}\,R$ を得る。
\end{definition}
...
\begin{tikzpicture}[
  ...
  % Dual order
  \node at (3, 2) {$\mathbb{N}^{\mathrm{op}}$（双対順序）};
  \foreach \n in {0,1,2,3}
  ...
  % Ideal powers (descending)
  \node at (3, -0.8) {$I^0 \supseteq I^1 \supseteq I^2 \supseteq \cdots$};
  \foreach \n in {0,1,2,3}
    \draw[->, dashed, gray] (M\n) -- (1.5+\n*0.9, -0.5);
\end{tikzpicture}
\caption{イデアル冪塔の構造。
  通常の $\mathbb{N}$ 上では反変（冪が増すにつれイデアルは縮小）なので，
  双対順序 $\mathbb{N}^{\mathrm{op}}$ を添字にとることで構造塔を構成する。}
\label{fig:ideal-pow}
\end{figure}
...
\begin{leancode}[caption={idealPowTower の定義（双対順序を活用）}]
...
% =============================================================================
\section{カテゴリ 8：シングルトンと層分解}\label{sec:singleton}
% =============================================================================
...
\]
すなわち自然数フィルトレーションは各層で厚み 1 の「殻」を積み上げた構造を持つ。
\end{proposition}
...
\begin{tikzpicture}[scale=0.9, every node/.style={font=\small}]
  ...
  \draw[<->, thick, accentcolor] (-0.4, 0) -- (-0.4, 2.6)
       node[midway, left=2pt] {$\lev_n$};
\end{tikzpicture}
\caption{natFiltration の層分解。各層 $\lev_n$ はシングルトン $\{n\}$ を積み上げた構造。}
\label{fig:layer}
\end{figure}
...
逆に $m = n$ なら $n \leq n$ かつ $n \not\leq n-1$（$n \geq 1$ より）。
Lean では \texttt{omega} タクティクが整数不等式を自動解決する。
\end{proofstrategy}

% =============================================================================
\section{カテゴリ 9：整数のフィルトレーション}\label{sec:integer}
% =============================================================================
...
\]
すなわち「$n$ 以下の絶対値を持つ偶数整数」の集合。
\end{definition}

\begin{proposition}
$\mathsf{almostFiltered}$ は単調増加：$m \leq n \implies
\mathsf{almostFiltered}(m) \subseteq \mathsf{almostFiltered}(n)$。
\end{proposition}

\begin{counterexample}[加法非閉性]\label{ex:not-add-closed}
$2, 2 \in \mathsf{almostFiltered}(2)$（$|2| = 2 \leq 2$，$2$ は偶数）。
しかし $2 + 2 = 4 \notin \mathsf{almostFiltered}(2)$（$|4| = 4 > 2$）。
\end{counterexample}

\begin{counterpointbox}
\texttt{almostFiltered} は単調性は持つが，加法について閉じていない。
これは「フィルトレーション = 代数的部分構造の族」という誤解を防ぐ例である。
フィルタリングされた環（filtered ring）では追加の代数条件が必要となる。
\end{counterpointbox}

% =============================================================================
\section{カテゴリ 10：最終的単調列と打ち切り}\label{sec:evmon}
% =============================================================================
...
\end{tikzpicture}
\caption{最終的単調列 \texttt{evMonExample} の概念図。
  $N=1$ 以降は単調増加。打ち切り塔 \texttt{truncateToTower} はこの部分を切り出す。}
\label{fig:evmon}
\end{figure}

% =============================================================================
\section{カテゴリ 11：有限塔と閾値塔}\label{sec:finite}
% =============================================================================

\subsection{閾値塔}

\begin{definition}[thresholdTower]\label{def:threshold}
重み関数 $w : \alpha \to \iota$ に対し，
\[
  \lev_{\mathsf{thr}}(i) := \{x \in \alpha \mid w(x) \leq i\}
\]
と定義される構造塔を \emph{閾値塔} と呼ぶ。
\end{definition}
...
\begin{tikzcd}[row sep=large, column sep=large]
  \alpha \arrow[dr, "w"'] \arrow[r, hook, "{\text{if } w(x) \leq i}"]
  & \lev_{\mathsf{thr}}(i) \arrow[d, hook] \\
  & \iota
\end{tikzcd}
\caption{閾値塔の構造。要素 $x \in \alpha$ は重み $w(x)$ が閾値 $i$ 以下のときに
  第 $i$ 層に属す。}
\label{fig:threshold}
\end{figure}

\begin{proposition}[閾値塔の加法閉性]\label{prop:threshold-add}
$(\iota, \vee)$ が半束，$(\alpha, +)$ が可換モノイドで，
$w : \alpha \to \iota$ が準加法的（$w(x+y) \leq w(x) \vee w(y)$）ならば，
$\lev_{\mathsf{thr}}(i)$ は加法について閉じている。
\end{proposition}
\begin{proof}
$w(x), w(y) \leq i$ より $w(x) \vee w(y) \leq i$，
準加法的条件より $w(x+y) \leq w(x) \vee w(y) \leq i$。
\end{proof}

\subsection{有限塔 finTower1, finTower2}

$\mathsf{Fin}\,3 = \{0, 1, 2\}$（添字）上の $\mathsf{Fin}\,5$（値）への塔を考える：

\begin{align*}
  \lev^{(1)}_i &:= \{x \in \mathsf{Fin}\,5 \mid x \leq 2i+1\}, \\
  \lev^{(2)}_i &:= \{x \in \mathsf{Fin}\,5 \mid x \leq 2i \wedge 2 \mid x\}.
\end{align*}

\begin{table}[H]
\centering
\caption{finTower1 と finTower2 の各層の要素}\label{tab:fintowers}
\begin{tabular}{c|c|c}
\toprule
層 $i$ & $\lev^{(1)}_i$ & $\lev^{(2)}_i$ \\
\midrule
$0$ & $\{0, 1\}$       & $\{0\}$ \\
$1$ & $\{0, 1, 2, 3\}$ & $\{0, 2\}$ \\
$2$ & $\{0,1,2,3,4\}$  & $\{0, 2, 4\}$ \\
...
\begin{theorem}
(1) $\union(\mathsf{finTower1}) = \mathsf{Fin}\,5$（全体）\\
(2) $\glob(\mathsf{finTower1}) = \{x \mid x \leq 1\}$\\
(3) $\forall i,\; \lev^{(2)}_i \subseteq \lev^{(1)}_i$（包含関係）\\
(4) $(\mathsf{inter}(T_1, T_2))_i = \lev^{(2)}_i$（交差は小さい方）
\end{theorem}

\begin{figure}[H]
\centering
\begin{tikzcd}[column sep=large, row sep=large]
  & \lev^{(1)}_2 = \mathsf{Fin}\,5 & \\
  \lev^{(2)}_2 = \{0,2,4\}
    \arrow[ur, hook]
  && \\
  & \lev^{(1)}_1 = \{0,1,2,3\}
  ...
  \lev^{(2)}_0 = \{0\}
    ...
    \arrow[uu, hook] &&
\end{tikzcd}
\caption{\texttt{finTower1}（右列）と \texttt{finTower2}（左列）の
  包含関係の Hasse 図。各レベルで $\lev^{(2)}_i \subseteq \lev^{(1)}_i$ が成立する。}
...
\begin{table}[H]
\centering
\caption{スーツテスト達成事項の総括}\label{tab:summary}
\begin{tabular}{rll}
\toprule
\# & 検証事項 & 主要な教訓 \\
\midrule
1 & 基本構成の正当性 & 定義の柔軟性（双対・交差） \\
2 & 空型での退化 & 空虚真の適切な扱い \\
3 & ランク一意性 & 部分順序の必要性 \\
4 & 無限添字（$\omega_0$）への拡張 & 有限・無限の境界処理 \\
5 & 単調性失敗の反例 & 公理の不可欠性 \\
6 & 前順序での非一意性 & 反対称律の役割 \\
7 & 代数との接続 & 双対順序の活用 \\
8 & 層の細かい構造 & フィルトレーションの幾何学 \\
9 & 代数閉性との分離 & フィルトレーション $\neq$ 部分代数 \\
10 & 漸近的単調性 & 打ち切りによる標準化 \\
11 & 有限・閾値塔 & 重み関数の汎用性 \\
\bottomrule
\end{tabular}
\end{table}

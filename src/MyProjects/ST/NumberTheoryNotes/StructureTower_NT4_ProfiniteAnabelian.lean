...
def truncateTower (T : StructureTower ℕᵒᵈ α) (n : ℕ) : StructureTower ℕᵒᵈ α where
  ...
  monotone_level := by
    ...
    exact Nat.min_le_min_left n (OrderDual.ofDual_le_ofDual.mpr hij)
...
theorem truncateTower_monotone (T : StructureTower ℕᵒᵈ α) {m n : ℕ} (hmn : m ≤ n) :
    NatInclusion (truncateTower T m) (truncateTower T n) := by
  intro k x hx
  apply T.monotone_level _ hx
  show min n (OrderDual.ofDual k) ≤ min m (OrderDual.ofDual k)
  exact Nat.min_le_min_right _ hmn
...
theorem truncateTower_coherent (T : StructureTower ℕᵒᵈ α) :
  ...
  intro n m hnm
  exact truncateTower_monotone T hnm
...
structure AnabelianCorrespondence
  ...
  faithful : ∀ d₁ d₂ : D,
    TowerIso (toTower d₁) (toTower d₂) → fromIso d₁ d₂
...
theorem HodgeTheaterData.ringTower_natInclusion_of_le
    ...
    NatInclusion (h.ringTowerAt 𝔮) (h.ringTowerAt 𝔭) := by
  intro n x hx
  exact Ideal.pow_le_pow_left hle _ hx
